...
This program does a depth first search for the min length cycle of a graph. Zero is output if no cycle is found.
...
CYCLE_DFS(int* usedList,int verts)
{
  ...
  */
  if(dasGraph[usedList[0]][lastVert] && (vertCount>=3)){
    if(vertCount<MIN_CYCLE)
      MIN_CYCLE=vertCount;
  }
  if(vertCount>=MIN_CYCLE)
    return;
