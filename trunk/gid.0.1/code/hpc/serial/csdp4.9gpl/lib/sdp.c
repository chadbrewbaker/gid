...
int sdp(n,k,C,a,constant_offset,constraints,byblocks,fill,X,y,Z,cholxinv,
	...
	Zi,O,rhs,dZ,dX,dy,dy1,Fp,nfree,freevarblocks,freevarind,printlevel,parameters)
     int n;
     int k;
...
#define BASIZE 100
  ...
  /*
   * Stuff for adjusting dX on problems with split free vars.
   */
	 ...
	 RETRYFACTOR:
	   ...
	   */
	   ...
	   if (nfree > 0)
	     {
	       if (printlevel >= 3)
		 printf("freevars: Adjusting affine dZ\n");
	       for (i=1; i<= nfree; i++)
		 {
		   block=freevarblocks[i];
		   j=freevarind[i];

		   if (printlevel >= 3)
		     {
		       printf("freevars before: Z(1) is %e\n",Z.blocks[block].data.vec[j]);
		       printf("freevars before: Z(2) is %e\n",Z.blocks[block].data.vec[j+1]);
		       printf("freevars before: dZ(1) is %e\n",dZ.blocks[block].data.vec[j]);
		       printf("freevars before: dZ(2) is %e\n",dZ.blocks[block].data.vec[j+1]);
		     };

		   if (-dZ.blocks[block].data.vec[j] > 0.99*Z.blocks[block].data.vec[j])
		     dZ.blocks[block].data.vec[j]=-0.99*Z.blocks[block].data.vec[j];
		   if (-dZ.blocks[block].data.vec[j+1] > 0.99*Z.blocks[block].data.vec[j+1])
		     dZ.blocks[block].data.vec[j+1]=-0.99*Z.blocks[block].data.vec[j+1];

		   if (printlevel >= 3)
		     {
		       printf("freevars after: dZ(1) is %e\n",dZ.blocks[block].data.vec[j]);
		       printf("freevars after: dZ(2) is %e\n",dZ.blocks[block].data.vec[j+1]);
		     };
	   ...
	   */
	   ...
	   if (nfree > 0)
	       ...
	       for (i=1; i<= nfree; i++)
		 {
		   ...
		   if ((dX.blocks[block].data.vec[j] > 0.0) &&
		       (dX.blocks[block].data.vec[j+1] > 0.0))
		     {
		       if (dX.blocks[block].data.vec[j] < dX.blocks[block].data.vec[j+1])
			 {
			   dX.blocks[block].data.vec[j+1]= dX.blocks[block].data.vec[j+1]-
			     dX.blocks[block].data.vec[j];
			   dX.blocks[block].data.vec[j]=0.0;
			 }
		       else
			 {
			   dX.blocks[block].data.vec[j]= dX.blocks[block].data.vec[j]-
			     dX.blocks[block].data.vec[j+1];
			   dX.blocks[block].data.vec[j+1]=0.0;
			 };
		     };
	   ...
	   if (relpinfeas < parameters.axtol)
	     alphap1=linesearch(n,dX,work1,work2,work3,cholxinv,workvec4,
				workvec5,workvec6,workvec7,mystepfrac,1.0,
				printlevel);
	   else
	     alphap1=linesearch(n,dX,work1,work2,work3,cholxinv,workvec4,
				workvec5,workvec6,workvec7,mystepfrac,1.0,
	   ...
	   */
	   ...
	   if (nfree > 0)
	     {
	       if (printlevel >= 3)
		 printf("freevars: Adjusting corrector dZ\n");
	       for (i=1; i<= nfree; i++)
		 {
		   block=freevarblocks[i];
		   j=freevarind[i];
	   ...
	   if (nfree > 0)
	       ...
	       for (i=1; i<= nfree; i++)
		 {
		   ...
		   difference=X.blocks[block].data.vec[j]-X.blocks[block].data.vec[j+1];

		   if (X.blocks[block].data.vec[j] > X.blocks[block].data.vec[j+1])
		     minx=X.blocks[block].data.vec[j+1];
		   else
		     minx=X.blocks[block].data.vec[j];		   


		   if (printlevel >= 3)
		     {
		       printf("freevars: X11 is %e \n",X.blocks[1].data.mat[0]);
		       printf("freevars before: X(1) is %e\n",X.blocks[block].data.vec[j]);
		       printf("freevars before: X(2) is %e\n",X.blocks[block].data.vec[j+1]);
		       printf("freevars before: X(1)-X(2) is %e\n",difference);
		       printf("freevars before: dX(1) is %e\n",dX.blocks[block].data.vec[j]);
		       printf("freevars before: dX(2) is %e\n",dX.blocks[block].data.vec[j+1]);
		     };

		   /*
		    * First center the difference.
		    */

		   if (dX.blocks[block].data.vec[j] > dX.blocks[block].data.vec[j+1])
		     mindx=dX.blocks[block].data.vec[j+1];
		   else
		     mindx=dX.blocks[block].data.vec[j];

		   dX.blocks[block].data.vec[j]=dX.blocks[block].data.vec[j]-mindx;
		   dX.blocks[block].data.vec[j+1]=dX.blocks[block].data.vec[j+1]-mindx;
	   ...
	   if (printlevel >= 3)
	     {
	       printf("refinement: Middle adjust dX Relative error in A(X+dX)=a is %e \n",relerr2);
	       if (relerr1 < relerr2)
		 printf("refinement: worse\n");
	       else
		 printf("refinement: better\n");
		   ...
		   /*
		    * Further, pull X down if needed.
		    */

		   if (minx > 1.00*fabs(difference))
		     {
		       /*
			*  Further adjust the steps to make x smaller
			*  overall.
			*/
		       dX.blocks[block].data.vec[j]=dX.blocks[block].data.vec[j]-0.75*minx;
		       dX.blocks[block].data.vec[j+1]=dX.blocks[block].data.vec[j+1]-0.75*minx;
		   ...
		   if (printlevel >= 3)
		     {
		       printf("freevars: X11 is %e \n",X.blocks[1].data.mat[0]);
		       printf("freevars: X(1) is %e\n",X.blocks[block].data.vec[j]);
		       printf("freevars: X(2) is %e\n",X.blocks[block].data.vec[j+1]);
		       printf("freevars: X(1)-X(2) is %e\n",difference);
		       printf("freevars: dX(1) is %e\n",dX.blocks[block].data.vec[j]);
		       printf("freevars: dX(2) is %e\n",dX.blocks[block].data.vec[j+1]);
