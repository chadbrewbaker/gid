...
Chad Brewbaker
  ...
  Output::Sum of edge weights where weight of edge xy is 1/(sqrt(deg(x)*deg(y)))
...
int main()
{
  ...
  while(1)
      ...
      for(i=0;i<verts;i++)
	{
	  ...
	  dasEdges[i]=edgeCount;
	}
      randic=0.0;
      for(i=0;i<verts;i++){
	for(j=i+1;j<verts;j++){
	  randic+= 1.0/sqrt((double)(dasEdges[i]*dasEdges[j]));
	}
      }
