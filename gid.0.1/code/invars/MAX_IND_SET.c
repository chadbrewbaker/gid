...
This program does a depth first search for the maximum set of verticies that share no edges.
...
void CLIQUE_DFS(int* usedList,int verts)
{
  ...
  if(vertCount>MAX_IND_SET)
    MAX_IND_SET=vertCount;
  if(MAX_IND_SET>= (vertCount + verts-(usedList[vertCount-1]+1)))
    return;
